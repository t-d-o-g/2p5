...
function deleteInactiveUser(inactiveUsers) {
  if (inactiveUsers.length > 0) {
    ...
    return admin.auth().deleteUser(userToDelete.uid).then(() => {
      console.log('Deleted user account from auth', userToDelete.uid, 'because of inactivity');

      db.collection('users').doc(userToDelete.uid).delete().then(function() {
        console.log('Deleted user account from db', userToDelete.uid, 'because of inactivity');
      }).catch(function(error) {
        console.error('Deletion of inactive user account from db', userToDelete.uid, 'failed:', error);
      });
      return null;
    }).catch(error => {
      console.error('Deletion of inactive user account from auth', userToDelete.uid, 'failed:', error);
      return null;
    });
  }
  return null;
